...
import { Clock, Calendar, CheckCircle, XCircle, ShoppingBag, FileText } from 'lucide-react';

export default function OrdersView({ orders }) {
  const [activeTab, setActiveTab] = useState('all'); // 'all' | 'delivered' | 'rejected'

  // 1. Core Filtering: Isolate completed history and exclude live pipeline tickets
  ...
  // Determine which filtered dataset array to map onto the table view UI cards
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10 px-4 pt-4 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-[24px] border border-gray-100 shadow-sm">
        ...
        {/* Dynamic Navigation Tabs Matrix */}
      ...
      {displayOrders.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {displayOrders.map(order => {
            ...
            return (
              <div key={order.id} className="bg-white rounded-[24px] border border-gray-100 shadow-sm overflow-hidden flex flex-col justify-between transition-all hover:border-gray-200/80">
                ...
                {/* Card Footer segment: Financial Metrics summaries layout panel */}
                <div className="px-5 py-4 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Grand Total Collected</span>
